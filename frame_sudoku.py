    ...
    def __init__(self, color, root, program_data):
        ...
        for x in range(0, 3):
            for y in range(0,3):
                ...
                self.button_dials[x][y] = ttk.Button(self.frameGen, image = self.dial_img[self.program_data.puzzle_dic["start"][x][y]][key])

                # if self.program_data.puzzle_dic["locked"][x][y] == 'x':
                #     self.button_dials[x][y] = ttk.Button(self.frameGen, image = self.dial_light[int(self.program_data.puzzle_dic["start"][x][y])-1][random.randint(0, 1)])
                # else:
                #     self.button_dials[x][y] = ttk.Button(self.frameGen, image = self.dial_light[int(self.program_data.puzzle_dic["start"][x][y])-1][random.randint(0, 1)])
